...
public class EntityCoordsSupport {
	...
	public static double getBottomHeightFromGround(Entity entity, final boolean ignoreLiquid) {
		return getHeightFromGround(entity, ignoreLiquid, entity.boundingBox.minY);
	}

	public static Point3D getEntityBlockSide(Entity entity, EntityPlayer entityPlayer) {

		MovingObjectPosition movingObjectPosition = getMovingObjectPosition(entity, entityPlayer);
	...
	public static int getEntityBlockSideHit(Entity entity, EntityPlayer entityPlayer) {
	...
	public static double getHeightFromGround(Entity entity, final boolean ignoreLiquid) {
		return getHeightFromGround(entity, ignoreLiquid, entity.posY);
	}

	public static Vec3 getLastTickPosition(Entity entity) {
		return Vec3.createVectorHelper(entity.lastTickPosX, entity.lastTickPosY, entity.lastTickPosZ);
	}
	...
	public static Vec3 getPrevPosition(Entity entity) {
		return Vec3.createVectorHelper(entity.prevPosX, entity.prevPosY, entity.prevPosZ);
	}
	...
	public static boolean rayTraceEntityBoxToEntityBox(Entity entity, Entity targetEntity) {
		return rayTraceEntityToEntity(entity, targetEntity, true)
				&& rayTraceEntityToEntity(entity, targetEntity, false);
	}

	public static boolean rayTraceEntityToEntity(Entity entity, Entity targetEntity, boolean bottom) {
		Vec3 vec3dSrc;
		Vec3 vec3dTarget;
		if (bottom) {
		...
		MovingObjectPosition mop = entity.worldObj.clip(vec3dSrc, vec3dTarget);

		return mop != null;
	}

	public static FixedDirection rotateFixedBoxDirection(Entity entity, boolean reverse) {
		float vZ;
		float vX;
	...
	public static Vec3 rotateFixedBoxDirection(Entity entity, double x, double y, double z) {
		FixedDirection fd = rotateFixedBoxDirection(entity, false);
		return entity.worldObj.getWorldVec3Pool().getVecFromPool(x * fd.dZ + z * fd.dX * fd.dT + y * fd.dX * -fd.dY,
				y * fd.dT + z * fd.dY, z * fd.dZ * fd.dT + x * -fd.dX + y * fd.dZ * -fd.dY);
	}

	public static ChunkCoordinates rotateFixedBoxDirection(Entity entity, int x, int y, int z) {
		return rotateFixedBoxDirection(entity, x, y, z, false);
	}

	public static ChunkCoordinates rotateFixedBoxDirection(Entity entity, int x, int y, int z, boolean reverse) {
		FixedDirection fd = rotateFixedBoxDirection(entity, reverse);
		return new ChunkCoordinates(x * fd.dZ + z * fd.dX * fd.dT + y * fd.dX * -fd.dY, y * fd.dT + z * fd.dY, z
				* fd.dZ * fd.dT + x * -fd.dX + y * fd.dZ * -fd.dY);
	}
	...
	private static MovingObjectPosition getMovingObjectPosition(Entity entity, EntityPlayer entityPlayer) {
		Minecraft mc = ModLoader.getMinecraftInstance();

		double d = mc.playerController.getBlockReachDistance();
		Vec3 playerPosV3d = entityPlayer.getPosition(1);
		playerPosV3d = playerPosV3d.addVector(-entity.posX, -entity.posY, -entity.posZ);
		playerPosV3d = Vec3.createVectorHelper(playerPosV3d.xCoord, playerPosV3d.yCoord, playerPosV3d.zCoord);

		Vec3 playerLookV3d = entityPlayer.getLookVec();
		playerLookV3d = playerPosV3d.addVector(playerLookV3d.xCoord * d, playerLookV3d.yCoord * d, playerLookV3d.zCoord
				* d);
		playerLookV3d = Vec3.createVectorHelper(playerLookV3d.xCoord, playerLookV3d.yCoord, playerLookV3d.zCoord);
		...
		MovingObjectPosition movingObjectPosition = Block.stone.collisionRayTrace(entity.worldObj, 0, 0, 0,
				playerPosV3d, playerLookV3d);
		return movingObjectPosition;
	}

	protected static double getHeightFromGround(Entity entity, final boolean ignoreLiquid, double posY) {
		if (posY < 1) {
			return -1;
		}
		...
		p = p.addY((double) -1);
		for (int i = 0; i < 256; i++) {
			boolean flag = doJobOr(entity, p.addY((double) -1), new Job() {
				@Override
				public boolean _do(Entity entity, NumPoint3D<?, ?> point3d) {
